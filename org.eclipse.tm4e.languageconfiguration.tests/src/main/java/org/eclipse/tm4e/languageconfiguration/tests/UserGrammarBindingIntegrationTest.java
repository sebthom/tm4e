...
class UserGrammarBindingIntegrationTest {
	...
	@Test
	void bundledLanguagesInheritTheSharedBaseAndEditorBinding() {
		final var manager = Platform.getContentTypeManager();
		final var sharedBase = castNonNull(manager.getContentType(GrammarContentType.BASE_TYPE_ID));
		final var packBase = castNonNull(manager.getContentType("org.eclipse.tm4e.language_pack.basetype"));
		final var language = castNonNull(manager.getContentType("org.eclipse.tm4e.language_pack.javascript"));
		assertThat(packBase.getBaseType()).isEqualTo(sharedBase);
		assertThat(language.getBaseType()).isEqualTo(packBase);
		// This runtime includes the real language pack; its languages must inherit across both parent levels.
		assertThat(castNonNull(PlatformUI.getWorkbench().getEditorRegistry().getDefaultEditor("example.js", language)).getId())
				.isEqualTo("org.eclipse.ui.genericeditor.GenericEditor");
		assertThat(manager.findContentTypesFor("ordinary.txt")).doesNotContain(sharedBase, packBase, language);
		final var declarations = Platform.getExtensionRegistry()
				.getConfigurationElementsFor("org.eclipse.core.contenttype.contentTypes");
		assertThat(declarations).filteredOn(element -> sharedBase.getId().equals(element.getAttribute("id")))
				.singleElement()
				.satisfies(element -> assertThat(element.getContributor().getName()).isEqualTo("org.eclipse.tm4e.registry"));
		assertThat(declarations).filteredOn(element -> packBase.getId().equals(element.getAttribute("id")))
				.singleElement()
				.satisfies(element -> assertThat(element.getContributor().getName()).isEqualTo("org.eclipse.tm4e.language_pack"));
	}
